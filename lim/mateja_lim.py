...
class LimPlots:
    ...
    def lams_overlay(self, absfac = 1.0, time = 4.0, shift = 0):
        ...
        cpexcelpath = "/mnt/c/Users/jmateja/Documents/SASVWcp/Messer_Sinclair_mastersheets/CP MiMES Fwd Dens Scan.xlsx"
        xl = pd.ExcelFile(cpexcelpath)
        # Maybe make new excel spreadsheet to help streamline this more
        #probe = '01W'
        #df_itf = pd.read_excel(xl, sheet_name = 'MCPL01W_190422_190423')  ######
        #df_otf = pd.read_excel(xl, sheet_name = 'MCPR01W_190422_190423')  ######
        df_itf = pd.read_excel(xl, sheet_name = 'MCPL02W_190424_190425')  ######
        df_otf = pd.read_excel(xl, sheet_name = 'MCPR02W_190424_190425')  ######
        #df_otf = pd.read_excel(xl, sheet_name = 'MCPR10W_190491_190492')  ######
        #df_otf = pd.read_excel(xl, sheet_name = 'DCPR02W')  ######
        #df_itf = pd.read_excel(xl, sheet_name = 'DCPL02W')  ######
        lams_itfx = df_itf['Distance along probe (cm)'].to_numpy()
        lams_otfx = df_otf['Distance along probe (cm)'].to_numpy()
        lams_itfy = medfilt(df_itf['W areal density (1e15cm-2)'].to_numpy(), 21) * 1e19/2
        lams_otfy = medfilt(df_otf['W areal density (1e15cm-2)'].to_numpy(), 21) * 1e19/2
        ...
        ax.plot(lim_itfx*100-shift, lim_itfnorm, '-', color=tableau20[6], ms = ms, linewidth = 4, label = "3DLIM ITF")
        ax.plot(lim_otfx*100-shift, lim_otfnorm, '-', color=tableau20[8], ms = ms, linewidth = 4, label = "3DLIM OTF")
        #ax.plot(lams_itfx, lams_itfnorm, color = 'r', alpha = 0.5)
        ...
        ax.set_ylabel("Normalized W Deposition", fontsize = 20, fontweight = "bold")
        ax.set_ylim(bottom = 0.0, top = 1.1)
        ax.set_xlim(left = 0-shift, right = 10)
        ax.set_xticks(x)
        ax.set_xticklabels(ticks)
        ax.legend(loc = 'upper right', fontsize = 15)
        ax.set_title("MCP 02W Deposition Profiles", fontsize = 20, fontweight = "bold")
