...
import load_gfile_d3d as loadg
import MDSplus as mds


def standalone(shot, time, psin=None, MiMES = True, tree='EFIT01', server='localhost'):
    pi = np.pi
    ...
    if psin is None:
        ...
        if MiMES:
            ...
            Zs_trunc = Zs < Z_axis
            f_psiN = scinter.Rbf(Rs[Zs_trunc], Zs[Zs_trunc], parmDICT['psiRZn'][Zs_trunc],
                                function='linear')
            """
            R_axis = parmDICT['RmAxis']
            Rs_trunc = Rs < R_axis
